...
type EnforceQL interface {
	Enforce(query string, allowedTenantLabelValues []string, tenantLabelName string, errorOnIllegalTenantValue bool) (string, error)
}
...
func enforceRequest(r *http.Request, enforce EnforceQL, allowedTenantLabelValues []string, tenantLabelName string, errorOnIllegalTenantValue bool, matchWord string) error {
	switch r.Method {
	case http.MethodGet:
		return enforceGet(r, enforce, allowedTenantLabelValues, tenantLabelName, errorOnIllegalTenantValue, matchWord)
	case http.MethodPost:
		return enforcePost(r, enforce, allowedTenantLabelValues, tenantLabelName, errorOnIllegalTenantValue, matchWord)
	default:
...
func enforceGet(r *http.Request, enforce EnforceQL, allowedTenantLabelValues []string, tenantLabelName string, errorOnIllegalTenantValue bool, matchWord string) error {
	log.Trace().Str("kind", "urlmatch").Str("queryMatch", matchWord).Str("query", r.URL.Query().Get("query")).Str("match[]", r.URL.Query().Get("match[]")).Msg("")

	query, err := enforce.Enforce(r.URL.Query().Get(matchWord), allowedTenantLabelValues, tenantLabelName, errorOnIllegalTenantValue)
	if err != nil {
		return err
	}
...
func enforcePost(r *http.Request, enforce EnforceQL, allowedTenantLabelValues []string, tenantLabelName string, errorOnIllegalTenantValue bool, matchWord string) error {
	...
	query, err := enforce.Enforce(query, allowedTenantLabelValues, tenantLabelName, errorOnIllegalTenantValue)
	if err != nil {
		return err
	}
